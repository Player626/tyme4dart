...
void main() {
  group('Event Tests', () {
    ...
    test('test0 - 生日', () {
      init();
      Event e = Event.fromName('公历生日')!;

      SolarDay d = e.getSolarDay(2008)!;
      expect(d.toString(), equals('2008年2月29日'));

      // 2005年没有2月29，按最初设置的，没有就倒推1天
      d = e.getSolarDay(2005)!;
      expect(d.toString(), equals('2005年2月28日'));

      e = Event.fromName('农历生日')!;
      d = e.getSolarDay(2026)!;
      expect(d.toString(), equals('2026年6月6日'));
    });

    test('test1 - 国际消费者权益日', () {
      init();
      Event e = Event.fromName('国际消费者权益日')!;

      SolarDay d = e.getSolarDay(2026)!;
      expect(d.toString(), equals('2026年3月15日'));
    });
  });
}
