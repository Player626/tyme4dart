...
class RabByungMonth extends MonthUnit {
  ...
  /// 特殊日数据 (年*13+月索引 -> 日数组)
  static final Map<int, List<int>> _days = {};
  ...
  RabByungMonth(int year, int month): super(year, month.abs()) {
    _initData();
    validate(year, month);
    leap = month < 0;
  }

  RabByungMonth.fromYm(int year, int month): this(year, month);
  ...
  @override
  String toString() => getRabByungYear().toString() + getName();

  @override
  RabByungMonth next(int n) {
    if (n == 0) {
      return RabByungMonth.fromYm(getYear(), getMonthWithLeap());
    }
  ...
  /// 首日
  RabByungDay getFirstDay() => RabByungDay(getYear(), getMonthWithLeap(), 1);

  /// 本月的藏历日列表
  List<RabByungDay> getDays() {
    List<RabByungDay> l = [];
    int y = getYear();
    int m = getMonthWithLeap();
    List<int> missDays = getMissDays();
    List<int> leapDays = getLeapDays();
    for (int i = 1; i < 31; i++) {
      if (missDays.contains(i)) {
        continue;
      }
      l.add(RabByungDay(y, m, i));
  ...
  /// 当月天数
  int getDayCount() => 30 + getLeapDays().length - getMissDays().length;

  /// 特殊日子列表（闰日为正，缺日为负）
  List<int> getSpecialDays() {
    List<int> days = _days[getYear() * 13 + getIndexInYear()] ?? [];
    return List.from(days);
  }

  /// 闰日列表
  List<int> getLeapDays() {
    return getSpecialDays().where((d) => d > 0).toList();
  }

  /// 缺日列表
  List<int> getMissDays() {
    return getSpecialDays().where((d) => d < 0).map((d) => -d).toList();
  }
}
